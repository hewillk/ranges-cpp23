...
class zip_view : public view_interface<zip_view<Views...>> {
  template<copy_constructible F, input_range... Uiews>
    requires((view<Uiews> && ...) && (sizeof...(Uiews) > 0) && is_object_v<F> &&
             ...
             can_reference<invoke_result_t<F&, range_reference_t<Uiews>...>>)
...
class adjacent_view : public view_interface<adjacent_view<V, N>> {
  template<forward_range U, copy_constructible F, size_t M>
    requires(view<U> && (N > 0) && is_object_v<F> &&
             ...
             can_reference<repeat_invoke_result_t<F&, range_reference_t<U>, M>>)
...
template<size_t N>
inline constexpr auto adjacent = []<viewable_range R>(R&& r) {
  if constexpr (N == 0)
    return views::empty<tuple<>>;
  else
    return adjacent_view<views::all_t<R>, N>(std::forward<R>(r));
};
...
template<forward_range V, copy_constructible F, size_t N>
  requires(view<V> && (N > 0) && is_object_v<F> &&
           ...
           can_reference<repeat_invoke_result_t<F&, range_reference_t<V>, N>>)
...
template<size_t N>
inline constexpr auto adjacent_transform = []<class F, viewable_range R>(R&& r, F&& f) {
  if constexpr (N == 0)
    return zip_transform(std::forward<F>(f));
  else
    return adjacent_transform_view<views::all_t<R>, decay_t<F>, N>(std::forward<R>(r),
                                                                   std::forward<F>(f));
};
